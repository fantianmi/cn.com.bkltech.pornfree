...
*查看用户的症状详情
*/
    public function zhengDetail(){
        $uid = I('uid','','intval');
        if(empty($uid)) $this->error('请选择查看的用户');
        /*获取昵称*/
        $nickname  = D('Member')->getNickName($uid);
        /*获取用户的症状*/
        $zhengList = D('ZhengzhuangUser')->getUserZheng($uid);
        /*获取症状的平均分和打分次数*/
        $ZhengzhuangScore = D('ZhengzhuangScore');
        foreach ($zhengList as &$val) {
            $arr = $ZhengzhuangScore->getAvgAndCount($val['zid'],$uid);
            $val = array_merge($val,$arr);
            // $val['scoreAvg'] = $ZhengzhuangScore->getZhengAvg($val['zid'],$uid);
        }
        $this->assign('list',$zhengList);
        $this->assign('nickname',$nickname);
        $this->assign('meta_title','用户症状');
        $this->display();
    }
/**
*用户的精虫上脑(戒撸历程)列表
*/
    public function lichengDetail(){
        $uid = I('uid','','intval');
        if(empty($uid)) $this->error('请选择查看的用户');
        /*获取昵称*/
        $nickname  = D('Member')->getNickName($uid);
        ...
        $map['uid'] = $uid;
        $list = $this->lists('Jingchong',$map,'ctime DESC');
        $this->assign('_list',$list);
        $this->display();
    }
    /**用户扩展资料信息页
