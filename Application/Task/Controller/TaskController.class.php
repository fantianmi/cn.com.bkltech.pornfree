...
class TaskController extends Controller{
	...
	*/
    public function getZheng(){
    	...
    	$result = M('zhengzhuang')->field($field)->where("type=0 and status=1")->order('sort')->select();
        foreach ($result as &$v) {
            $v['type'] = "sys";
        }
    	if(!empty($uid)){
    		// $user = M('zhengzhuang_user')->field('zid,uid')->where("uid={$uid}")->select();
            $user = M('zhengzhuang')->field($field)->where("uid={$uid}")->select();
            if($user){
                foreach ($user as &$v) {
...
/*上面用户创建症状处理拆出来的接口*/
    public function addUserZheng(){
        $zid   = I('zid');
        $uid   = I('uid');
        if(empty($zid) || empty($uid)) exit(err(100));
        $zid   = explode(",",$zid);
...
*向数据表插入（ 为了app统计图 ）
*/
    public function addZhengScoreRecord($uid,$zid,$score){
        ...
        if($re){//更新
            ...
            $data = array(
                ...
                'zid'         => $zid,
                'score'       => $score,
                'create_time' => strtotime(date('Ymd',NOW_TIME)),
            );
            $model->add($data);
        }
    }
...
*/
    public function getInfo(){
        ...
        $list = M('document')->field($field)->where("category_id={$category_id} and status=1")->page($pagenum,$pagesize)->select();
        foreach ($list as &$v) {
