...
class TalkMessageModel extends Model
{
    ...
    public function addMessage($content, $uid, $talk_id)
    {
        ...
        $this->sendMessagePush($talk, $message, $uid);
     ...
     * @param $talk_id 消息id
     */
    public function sendMessage($content, $to_uids, $talk_id,$uids)
    {
        foreach ($to_uids as $uid) {
             ...
             */
            if ($uid != $uids) {
                D('Message')->sendMessage($uid, '对话内容：' . op_t($content), '您有新的聊天消息', U('UserCenter/Message/talk', array('talk_id' => $talk_id)), $uids, 1);
            }
        }
    }
     ...
     * @auth 陈一枭
     */
    private function sendMessagePush($talk, $message ,$uid)
    {
