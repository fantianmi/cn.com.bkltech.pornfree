 ...
 * @param int    $uid     发起人id
 */
    public function hg_createTalk($members, $message = '',$uid=0)
    {
        // 如果是有效的连天对象就不创建了
        $check1 = $this->where("uids='[{$uid}],[{$members[0]}]' and status=1")->find();
        $check2 = $this->where("uids='[{$members[0]}],[{$uid}]' and status=1")->find();
        ...
        if (is_array($members)) {
            ...
            /*创建talk*/
            $talk['uids'] = implode(',', array('[' . $uid . ']', '[' . $members . ']'));
        }
        if ($message != '') {
            $talk = $this->assignTalkData($message, $talk);
            //通过消息获取到对应应用内的消息模型
            ...
            if (count($orin_member) == 1) {
                $user_one = query_user(array('nickname'), $orin_member[0]);
                $user_two = query_user(array('nickname'), $uid);
                $talk['title'] = $user_one['nickname'] . ' 和 ' . $user_two['nickname'] . '的聊天';
            }
        }
        ...
        //获取图标用于输出
        // $talk['avatar']   = $ico_user['avatar64'];
        // $talk['nickname'] = $user_one['nickname'];
        return $talk;
        /*创建talk end*/
    }
// 私信推送
