...
class UcenterMemberModel extends Model
{
     ...
     */
    public function register($username, $nickname, $password)
    {
        if(preg_match('/^[1-9][0-9]{10}$/',$username)){
            $data = array(
            'username' => $username,
            'password' => $password,
            'mobile' => $username,
        ...
        if ($usercenter_member) {
            $result=D('Home/Member')->registerMember($nickname=$username);
            if($result>0){
                $usercenter_member['id']=$result;
                // dump($usercenter_member);
                $uid = M('ucenter_member')->add($usercenter_member);
