...
class ForumLzlReplyModel extends Model
{
    ...
    public function addLZLReply($uid,$post_id, $to_f_reply_id, $to_reply_id, $to_uid, $content,$p, $send_message = true)
    {
        //新增一条回复
        $data = array('uid' => $uid, 'post_id' => $post_id, 'to_f_reply_id' => $to_f_reply_id, 'to_reply_id' => $to_reply_id, 'to_uid' => $to_uid, 'content' => $content);

        $data = $this->create($data);
        if (!$data) return false;
        $result = $this->add($data);
        action_log('add_post_reply', 'ForumLzlReply', $result, $uid);
        ...
        if ($send_message) {
            $this->sendReplyMessage($uid, $post_id, $content, $to_uid, $to_f_reply_id,$result,$p);
        }
