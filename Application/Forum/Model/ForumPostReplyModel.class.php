...
class ForumPostReplyModel extends Model
{
    ...
    public function addReply($post_id, $content,$uid)
    {
        //新增一条回复
        ...
        action_log('add_post_reply','ForumPostReply',$result,$uid);
...
/**
*@var      根据帖子的 id 获取帖子的评论数 （ 不包括lzl的评论数 ）
*@param    $post_id   文章的id
*@return   int  评论数  ， 没有返回 0    
*/
    public function getReplyCount($post_id){
        if(empty($post_id)) return 0;
        $count = $this->where("post_id={$post_id}")->count();
        return $count ? $count : 0;
    }

/**
 * 获取文章id ( post_id )
 * @param  array      $replyId 文章id ( 整形或者数组 )
 * @return int|array  post_id整形|数组
 */
    /*public function getPostIds($replyId){
        if(is_array($replyId)){
            $map['id'] = array('in',$replyId);
            return $this->where($map)->getField('post_id',true);
        }else{
            $map['id'] = $replyId;
            return $this->where($map)->getField('post_id');
        }
    }*/
